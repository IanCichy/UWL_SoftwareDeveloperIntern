...
public partial class wiaa_Controls_ReuterRoomControl : System.Web.UI.UserControl
{
    ...
    protected void Room_Click(object sender, CommandEventArgs e)
    {
        try
        {
            ...
            {
                String message = "Room " + roomNum + " has already been reserved by:";
                String bedA = SqlHelper.ExecuteScalar(Settings.WIAAConnection, CommandType.Text, "SELECT [Bed A] FROM VolunteerRoomReservations WHERE Room = '" + roomNum + "'").ToString();
                String bedB = SqlHelper.ExecuteScalar(Settings.WIAAConnection, CommandType.Text, "SELECT [Bed B] FROM VolunteerRoomReservations WHERE Room = '" + roomNum + "'").ToString();
                String bedC = SqlHelper.ExecuteScalar(Settings.WIAAConnection, CommandType.Text, "SELECT [Bed C] FROM VolunteerRoomReservations WHERE Room = '" + roomNum + "'").ToString();
                String bedD = SqlHelper.ExecuteScalar(Settings.WIAAConnection, CommandType.Text, "SELECT [Bed D] FROM VolunteerRoomReservations WHERE Room = '" + roomNum + "'").ToString();
                if (!bedA.Equals("")) message += "\\n     " + bedA;
                if (!bedB.Equals("")) message += "\\n     " + bedB;
                if (!bedC.Equals("")) message += "\\n     " + bedC;
                if (!bedD.Equals("")) message += "\\n     " + bedD;

                Security.ShowAlertMessage(message);
            }
            else if (btnRoomClicked.BackColor == Color.Yellow)
            {
                ...
                String bedC = SqlHelper.ExecuteScalar(Settings.WIAAConnection, CommandType.Text, "SELECT [Bed C] FROM VolunteerRoomReservations WHERE Room = '" + roomNum + "'").ToString();
                String bedD = SqlHelper.ExecuteScalar(Settings.WIAAConnection, CommandType.Text, "SELECT [Bed D] FROM VolunteerRoomReservations WHERE Room = '" + roomNum + "'").ToString();
                if (!bedA.Equals("") || !bedB.Equals("") || !bedC.Equals("") || !bedD.Equals(""))
                    btnRoomClicked.BackColor = Color.LightGreen;
                else
